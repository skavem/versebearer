<script lang="ts">
  ...
  async function pick(id: string) {
    open = false;
    // HIGH №3 обзора: карточка уже выбранного устройства выглядит и ведёт
    // себя как кнопка — оператор открывает модалку посмотреть частоту и
    // кликает по подсвеченной карточке, не для того чтобы поменять
    // устройство. SetDevice физически обрывает звук, так что для
    // совпадающего id в Go вообще не ходим (бэк тоже защищён — см.
    // audio_devices.go — но здесь дешевле не открывать лишний Wails-вызов).
    if (id === devices.selectedId) return;

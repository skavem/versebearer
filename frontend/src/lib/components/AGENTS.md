...
| `PlaylistSidebar.svelte` | Left third of the "Звук" tab: which playlist is active, plus everything done to a playlist *as a row* — create, inline rename, delete (with its own confirm modal). Split out of `PlaylistPanel.svelte`, which had grown to two halves sharing nothing but `audioStore.playlists`; both read that store directly, so nothing is passed between them. "Would deleting this stop the sound in the room?" comes from `audioStore.isPlaylistOnAir(id)` — in the store, not duplicated here, because `PlaylistPanel.svelte`'s "Удалить всё" modal asks the same question and the two answers must not drift |
| `PlaylistPanel.svelte` | The active playlist's *contents*: flags (autoadvance/loop/fade), item list (import, remove, double-click to play) plus a single up/down button pair beside the list — mirroring `CoupletsList.svelte`'s side toolbar — that moves whichever item `selectedItemId` points at. Reordering computes the target array order client-side and sends it whole via `ReorderPlaylist`, unlike `CoupletsList.svelte`'s pairwise-swap reorder. Takes a bindable `selectedItemId` prop so `Audio.svelte`'s keyboard handler and the reorder buttons share one row highlight independent from "currently playing" (blue = keyboard/mouse selection, amber = on air — two different facts about a row, never the same color) |
| `MiniPlayer.svelte` | Transport bar for the "Звук" tab: prev/play-pause/next/stop (centered), a seekable progress range with a "-remaining" label (highlighted red in the last 10s), a volume range, and a peak-level meter — timeline and volume/level rows share one grid column template so they line up. Takes the same `selectedItemId` as `PlaylistPanel.svelte`: the play/pause button starts the selected item when nothing is loaded (`audioStore.playOrToggleSelected`, also used by the tab's Space handler so the two never diverge); otherwise it only toggles the already-loaded track (И5, "exactly one active track"). The level meter's peak-hold is a local `requestAnimationFrame` exponential decay layered over the polled `PlayerState.peak`, not a second backend channel |

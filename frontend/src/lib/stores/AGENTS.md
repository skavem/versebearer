...
- Wails event names from `dbHandler.go` (`show_verse`, `hide_verse`, `show_couplet`, `hide_couplet`, `songs_update`, `song_update`) plus `translations_update` from `db_import.go` — keep in sync with the Go side.

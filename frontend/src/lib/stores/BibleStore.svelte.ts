...
import { Chapter, Translation, Verse, type Book } from '$lib/bindings/changeme/backend/models';
import { GetBooks, GetChapters, GetVerses } from '$lib/bindings/changeme/dbhandler';
...
const createBibleStore = () => {
	...
	const chapters = {
		...
		next() {
			let ind = chaptersList.findIndex((c) => c.ID === activeChapter?.ID);
	...
	const verses = {
		...
		next() {
			let ind = versesList.findIndex((v) => v.ID === activeVerse?.ID);

 ...
 *
 * `Match.start`/`len` приходят с Go в символах (не в байтах) — см. комментарий
 * к типу Match на бэкенде. Весь текст переводов лежит в BMP, где символ Unicode
 * и единица UTF-16 совпадают, поэтому позиции применяются к строке напрямую
 * через slice.
 */

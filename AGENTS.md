...
- The dev watcher only watches `*.go` (`dev_mode.watched_extension`). That is correct *because* the frontend is live over vite — Go changes are the only ones needing a rebuild. Note that setting a `.go` file's mtime (PowerShell `LastWriteTime`) does **not** wake it, measured twice against a live watcher; use a real content change to force a rebuild.

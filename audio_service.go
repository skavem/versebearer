...
func NewAudioService() *AudioService {
	...
	// inits.DB готова к этому моменту (база открывается в main() вызовом
	// inits.Open() до создания сервисов — см. main.go). Ошибка чтения — не
	// повод падать: откатываемся на полную
...
// PlayerState — то, что фронт вкладки «Звук» опрашивает раз в 500 мс.
...
//
// ⚠️ Порядок вызовов (MEDIUM №5 обзора): сперва a.pl.stop() — тишина
// немедленно, — и только потом dropPending(). Раньше было наоборот:
// dropPending() блокируется на <-pn.ready, если "следующий" ещё
// декодируется (первые ~0.5с трека — окно, когда автопереход только успел
// открыть следующий файл), и оператор слышал недодоигранный кусок текущего
// трека, пока Стоп ждал чужой decodeExt. pn.canceled гарантирует, что
// запоздалый декодер ничего не установит, так что откладывать dropPending
// безопасно.
...
func (a *AudioService) FadeOutStop() {
	...
	// MEDIUM №5 обзора: dropPending() блокируется на <-pn.ready, пока
	// "следующий" декодируется — раньше он звался здесь БЕЗУСЛОВНО, ДО
	// решения, каким путём пойдём. В ветках ниже, что делегируют в Stop()
	// (нечего фейдить / трек на паузе / FadeMs<=0), это откладывало
	// немедленную тишину Stop() на то же самое время, которое сам Stop()
	// теперь честно ставит первым делом (см. его комментарий) — блокировка
	// просто переезжала на строчку раньше и сводила реордер в Stop() на
	// нет. dropPending() сдвинут ниже, к единственному пути, где он
	// действительно нужен ДО пересборки цепочки: настоящему фейду.
	...
	a.dropPending() // оператор явно останавливает воспроизведение (фейдом) — заранее подготовленный "следующий" больше не актуален
